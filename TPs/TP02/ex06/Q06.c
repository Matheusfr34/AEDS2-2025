...
int procuraMenor(SHOW *array, int len, int i, int menor, int *comp){
	if(i < len){
		char *menorTitle = toLowerCase(array[menor].title);

		char *iTitle = toLowerCase(array[i].title);

		*comp+=1;
		if(strcmp(menorTitle,iTitle) > 0){
			menor = procuraMenor(array, len,  i + 1, i,comp);
		}else{
			menor = procuraMenor(array, len, i + 1,  menor,comp);
		}
...
void ordenaSelecaoRec(SHOW *array, int len, int i, int *mov, int *comp){
	int menor = i;
	if(i < len - 1){
		menor = procuraMenor(array,len,i + 1,menor, comp);
		...
		ordenaSelecaoRec(array,len,++i,mov,comp);
	}
}
int main(){
	...
	ordenaSelecaoRec(array,tam_array, 0,&mov,&comp);
	clock_t fim = clock();
	double duration = ((double)(fim - inicio)) / CLOCKS_PER_SEC;

	FILE *log = fopen("./853431_selecaoRecursiva.txt","w");

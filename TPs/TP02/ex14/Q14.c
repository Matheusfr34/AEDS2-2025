...
bool verificaShow(SHOW tmp, SHOW j, int *comp){
	int tmpYear = tmp.release_year;
	int jYear = j.release_year;
	char *tmpTitle = toLowerCase(tmp.title);
	char *jTitle = toLowerCase(j.title);


	bool v2 = (tmpYear == jYear &&
...
int getMax(SHOW *array, int len){
	int maior = array[0].release_year;
	for(int i = 0; i < len; i++){
		if(maior < array[i].release_year){
...
SHOW* countingsort(SHOW *array, int len, int exp, int *mov, int *comp){
	SHOW *saida = (SHOW *)calloc(len,sizeof(SHOW));  
	int count[10] = {0};

	for(int i = 0; i < len; count[(array[i].release_year / exp) % 10]++ , i++);

	for(int i = 1; i < 10; count[i] += count[i - 1], i++);

	for(int i = len - 1; i >= 0; i--){
		(*mov)++;
		saida[count[(array[i].release_year / exp) % 10] - 1] = clone(array[i]);
...
void ordenaRadix(SHOW *array, int len,int *mov, int *comp){
	int max = getMax(array, len);
	SHOW *saida;
	for(int exp = 1; max / exp > 0;exp *= 10){
		saida = countingsort(array, len, exp, mov, comp);
	}

	//Inserção pra ajustar os titulos, uma vez que o radix não é capaz de lidar com string
	for(int i = 1;i < len; i++){
		SHOW tmp = saida[i];
		int j = i - 1;
		while(j >= 0 && verificaShow(tmp,saida[j],comp)){
			(*mov)++;
			saida[j + 1] = saida[j];
			j--;
		}
...
int main(){
	...
	ordenaRadix(array,tam_array, &mov,&comp);
	clock_t fim = clock();
	double duration = ((double)(fim - inicio)) / CLOCKS_PER_SEC;

	FILE *log = fopen("./858190_radixsort.txt","w");

...
int compareShows(SHOW a, SHOW b, int *comp) {
    (*comp)++;
...
void heapify(SHOW *arr, int n, int i, int *comp) {
    int menor = i;
    int esq = 2 * i + 1;
    int dir = 2 * i + 2;

    if (esq < n && compareShows(arr[esq], arr[menor], comp) > 0)
        menor = esq;

    if (dir < n && compareShows(arr[dir], arr[menor], comp) > 0)
        menor = dir;

    if (menor != i) {
        swap(&arr[i], &arr[menor]);
        heapify(arr, n, menor, comp);
    }
}

void partialHeapSort(SHOW *arr, int n, int k, int *mov, int *comp) {
...
int main(){
	...
	partialHeapSort(array,tam_array,10, &mov,&comp);
	clock_t fim = clock();
	double duration = ((double)(fim - inicio)) / CLOCKS_PER_SEC;

	FILE *log = fopen("./853431_heapsortParcial.txt","w");
